...
## 17.3 常用操作示例(不属于原官方文档)

以下示例基于一个包含两个 TAP 的菊花链环境：
*   **tap1**: 靠近 TDO，ID `0x20000001`，IR 长度 4 位。
*   **tap2**: 靠近 TDI，ID `0x10000001`，IR 长度 4 位。

### 示例 1: 切换指令寄存器 (IR Scan)

让 `tap1` 进入用户自定义模式（假设指令码为 `0x2`），同时让 `tap2` 进入 BYPASS 模式（指令码 `0xF`）。

```tcl
# 语法: irscan [TAP名称] [指令值] [TAP名称] [指令值] ...
irscan tap1.tap 0x2 tap2.tap 0xF
```

### 示例 2: 读写数据寄存器 (DR Scan)

向 `tap1` 的 8 位数据寄存器写入 `0xAA`，并读取旧值。此时 `tap2` 处于 BYPASS 模式（1 位寄存器）。

```tcl
# 语法: drscan [TAP名称] [位数] [写入值] [TAP名称] [位数] [写入值] ...
# 注意：虽然 tap2 在 BYPASS，但为了严谨，通常只列出目标 TAP，OpenOCD 会自动处理其他 TAP 的 BYPASS 位。
# 但如果手动控制，需要知道 BYPASS 寄存器是 1 位的。

# 简单方式（推荐）：只操作目标 TAP，OpenOCD 自动处理其他 TAP
drscan tap1.tap 8 0xAA
```

### 示例 3: 同时操作两个 TAP

假设两个 TAP 都处于用户模式（指令 `0x2`），且都有 8 位数据寄存器。我们想给 `tap1` 写 `0x11`，给 `tap2` 写 `0x22`。

```tcl
# 1. 先设置指令
irscan tap1.tap 0x2 tap2.tap 0x2

# 2. 再写入数据
# 注意：参数顺序通常与 openocd.cfg 中定义的 TAP 顺序一致
drscan tap1.tap 8 0x11 tap2.tap 8 0x22
```

### 示例 4: 使用 `runtest` 等待操作完成

某些操作（如 Flash 编程或 BIST 自测试）在 Update-DR 后需要一定时间才能完成。

```tcl
# 1. 启动操作（假设写入 0x1 启动自测）
drscan tap1.tap 8 0x1

# 2. 在 RUN/IDLE 状态等待 1000 个 TCK 周期
runtest 1000

# 3. 检查结果
drscan tap1.tap 8 0x0
```